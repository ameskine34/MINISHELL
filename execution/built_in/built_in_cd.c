/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   built_in_cd.c                                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ameskine <ameskine@student.1337.com>       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/23 11:23:48 by ameskine          #+#    #+#             */
/*   Updated: 2025/07/19 20:33:19 by ameskine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/built_in.h"

void pwd_changes(t_list *envi, char *current, char *new)
{
    while (envi)
    {
        if (!ft_strncmp("PWD", ((t_env*)envi->content)->key, 3))
        {
            free(((t_env *)envi->content)->value);
            ((t_env *)envi->content)->value = ft_strdup(new);
        }
        else if (!ft_strncmp("OLDPWD", ((t_env*)envi->content)->key, 6))
        {
            free(((t_env *)envi->content)->value);
            ((t_env *)envi->content)->value = ft_strdup(current);
        }
        envi = envi->next;
    }
}

void    handle_cd_no_args(t_list *envi)
{
    char *current;
    char *new;

    current = getcwd(NULL, 0); // will be the new OLDPWD
    if (!chdir(getenv("HOME")))
    {
        new = getcwd(NULL, 0); // will be the new PWD
        pwd_changes(envi, current, new);
        free(current);
        free(new);
    }
    else
    {
        write(2, "bash: cd: HOME not set or invalid directory\n", 43);
        free(current);
        exit(1);
    }
}

void    handle_cd_one_arg(t_list *lst, t_list *envi)
{
    char *current;
    char *new;

    current = getcwd(NULL, 0); // will be the new OLDPWD
    if (!chdir(lst->next->content))
    {
        new = getcwd(NULL, 0); // will be the new PWD
        pwd_changes(envi, current, new);
        free(current);
        free(new);
    }
    else if (!ft_strncmp("..", lst->next->content, 2) && chdir(lst->next->content))
    {
        write(2, "cd: error retrieving current directory: getcwd: cannot access parent directories: No such file or directory", 107); // i have to write in the stderror
        free(current);
        exit(0);
    }
    else
    {
        write(2, "bash: cd: ", 10);
        write(2, (char *)lst->next->content, ft_strlen((char *)lst->next->content));
        write(2, " No such file or directory\n", 26);
        free(current);
        exit(1);
    }
}

void    handle_cd_too_many_args()
{
    write(2, "bash: cd: too many arguments", 28);
    exit(1); //i have to return till running the minishell again 
}

void    ft_cd(t_list *lst, t_list *envi)
{
    int k;

    k = ft_lstsize(lst);
    if (k == 1)
        handle_cd_no_args(envi);
    else if (k == 2)
        handle_cd_one_arg(lst, envi);
    else if (k > 2)
        handle_cd_too_many_args();
}